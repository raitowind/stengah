...
# // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Raizhel ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Author : ${green}╭────────────Julak_Bantur®────────────╮${NC}${YELLOW}(${NC} ${green} Raizhel ${NC}${YELLOW})${NC}"
echo -e " © Recode By My Self Raizhel${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/raitowind/raizhel/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/raitowind/raizhel/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/raitowind/raizhel/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/raitowind/raizhel/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/raitowind/raizhel/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/raitowind/raizhel/main/ip | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/raitowind/raizhel/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
function menu(){
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/raitowind/raizhel/main/limit/menu.zip
    unzip menu.zip
    chmod +x menu/*
